...
| Element refusals, scroll containers, audit, capture failures | *UI Inspector — Compose reliability fixtures* | One tab per fixture group: repeated and disabled targets, a feed of carousels beside two unrelated lists, audit faults, a UI that never goes idle, a hybrid screen, a short column with one row cut by its edge and one below the fold, and things that appear, disappear, become enabled or switch on 3 s after a button, for waits. Calls and expected answers: `docs/COMPOSE-SUPPORT-PLAN.md`, *Device checks*, also behind each fixture card's info button |
