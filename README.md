...
It also includes an **Android MCP server** for Claude Code, Claude Desktop, Cursor, and other MCP clients. Agents get 58 strongly typed Android tools instead of unrestricted shell access, while destructive operations remain approval-gated and visible in the IDE.
...
- device conditions for background work: Doze, App Standby buckets, battery, each with a reset
- HTTP proxy state
...
- Copy MCP client configuration (stdio or HTTP)
- Install MCP client configuration into this project
- Rotate MCP token
