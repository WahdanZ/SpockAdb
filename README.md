...
It also ships an <b>Android MCP server</b>: give Claude Code, Claude Desktop, Cursor or any MCP client safe, structured access to a connected device. 43 strongly typed tools rather than a raw shell, and anything destructive asks you first, every time.
...
- **43 strongly typed tools** instead of a raw shell, so an agent can reason about what an
