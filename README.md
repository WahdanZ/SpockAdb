...
Spock ADB brings ADB workflows into one shared tool window that keeps a selected device and app as the active target across Device, Timeline, Storage, Logcat, Commands, UI Inspector, and Background Work — plus a built-in MCP server exposing 67 strongly typed Android tools for Claude Code, Claude Desktop, Cursor, and other AI clients.
...
It provides a shared debugging workspace where **Device, Timeline, Storage, Logcat, Commands, UI Inspector, Background Work, and MCP** all use the same selected Android device and application.
